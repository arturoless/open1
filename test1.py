...
find = False 
for filas in range(len(src1)-len(imCrop)):
    for columnas in range(len(src1[0])-len(imCrop[0])):
        for filas_crop in range(len(imCrop)):
            for columnas_crop in range(len(imCrop[0])):
                temp_sad=np.sum(abs(np.subtract(imCrop[filas_crop][columnas_crop],src1[filas+filas_crop][columnas+columnas_crop])))
